...
#
# Tres cosas, y las tres se midieron contra el sitio real:
#
# 1. Se pide GZIP. Una pagina del catalogo pesa 1,27 MB en crudo y 172 KB
#    comprimida (7,4 veces menos). Barrer las categorias del usuario son ~390
#    paginas: 500 MB contra 67 MB.
# 2. Se REUSA la conexion. Antes cada pedido abria una conexion nueva, con su
#    saludo TLS de ~0,3 s. Por 390 paginas eran minutos tirados en saludar.
# 3. Hay un tope GLOBAL de pedidos simultaneos (RITMO). Carrefour limita el
#    ritmo: medido, con 8 o con 24 en paralelo entrega lo mismo (~0,55
#    paginas/s en frio). Pasado ese punto solo se acumulan esperas y errores.
#    Que el tope sea global es lo que permite que la pantalla pida varias
#    categorias a la vez sin castigar al servidor.
# ----------------------------------------------------------------------
RITMO = 8
_RITMO = threading.BoundedSemaphore(RITMO)

_HOST = urllib.parse.urlsplit(BASE).netloc
_LIBRES = queue.LifoQueue()      # conexiones abiertas, listas para reusar
_CTX_ACTUAL = [CTX]              # se degrada a "sin verificar" si los certificados fallan
...
def _conexion():
    """Una conexion libre, o una nueva si no hay ninguna a mano."""
    try:
        return _LIBRES.get_nowait()
    except queue.Empty:
        return http.client.HTTPSConnection(_HOST, context=_CTX_ACTUAL[0], timeout=30)


def _una_vuelta(c, camino, cab):
    c.request("GET", camino, headers=cab)
    r = c.getresponse()
    crudo = r.read()                      # hay que leerlo entero para poder reusar
    if (r.headers.get("Content-Encoding") or "").lower() == "gzip":
        crudo = gzip.decompress(crudo)
    if r.status >= 400:
        raise OSError("HTTP %d %s" % (r.status, r.reason))
    return crudo, r.headers


def pedir(url):
    """Devuelve (datos, cabeceras). Las cabeceras traen `resources`, que dice
    cuantos productos tiene la consulta entera: con eso se sabe de antemano
    cuantas paginas hay que pedir, en vez de descubrirlo a los tropezones."""
    partes = urllib.parse.urlsplit(url)
    camino = partes.path + (("?" + partes.query) if partes.query else "")
    cab = {
        "User-Agent": UA,
    ...
    with _RITMO:
        for _ in range(3):
            c = _conexion()
            try:
                crudo, cabezas = _una_vuelta(c, camino, cab)
            except ssl.SSLError as e:
                # Certificados del sistema desactualizados: se sigue sin
                # verificar en vez de romper toda la herramienta.
                _cerrar(c)
                _CTX_ACTUAL[0] = ssl._create_unverified_context()
                ultimo = e
                continue
            except Exception as e:
                # Casi siempre es una conexion reutilizada que Carrefour ya
                # habia cerrado de su lado. Se tira y se prueba con una nueva;
                # un GET se puede repetir sin consecuencias.
                _cerrar(c)
                ultimo = e
                continue
            _LIBRES.put(c)
            return json.loads(crudo.decode("utf-8", "replace")), cabezas
    raise ultimo if ultimo else OSError("No se pudo consultar %s" % url)
...
def total_informado(cabeceras):
    """Carrefour contesta `resources: 0-49/865`: el 865 es el total de la
    consulta. Es gratis —viene en la misma respuesta— y evita pedir paginas
    a ciegas hasta que una vuelve incompleta."""
    res = (cabeceras.get("resources") or "") if cabeceras else ""
    m = re.search(r"/\s*(\d+)\s*$", res)
...
def pagina_categoria(cat_id, pagina):
    ...
    Devuelve (productos, total_de_la_categoria). El total sale de la cabecera
    `resources` de la misma respuesta, asi que no cuesta un pedido extra."""
    ...
    d, cabezas = pedir(url)
    return (d if isinstance(d, list) else []), total_informado(cabezas)
...
def explorar_ofertas(cat_id, desde=0, paginas=EN_PARALELO):
    ...
    productos = 0
    total = None               # cuantos productos tiene la categoria entera
    ...
    # Cuantas paginas tiene la categoria, ya sabiendo el total. La pantalla usa
    # esto para pedir todos los tramos que faltan de una vez (y para que la
    # barra de progreso diga la verdad en vez de estimar).
    paginas_total = None
    if total is not None:
        paginas_total = min((total + POR_PAGINA - 1) // POR_PAGINA, PAGINAS_MAX)
        if desde + len(tanda) >= paginas_total:
            fin = True

    return {"cat": cat_id, "items": items, "error": error, "fin": fin,
            "leidas": len(tanda), "productos": productos,
            "total": total, "paginas": paginas_total}
